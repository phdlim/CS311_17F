...
void IF_Stage()
{
    ...
    {
        if(CURRENT_STATE.PIPE[IF_STAGE])
            CURRENT_STATE.PIPE[IF_STAGE] = CURRENT_STATE.PC;
        return;
    }
    uint32_t cur_pc = CURRENT_STATE.PC;
        ...
        CURRENT_STATE.ID_EX_RT == CURRENT_STATE.IF_ID_RT)))
    {
        ...
        else
        {
            ...
            else
            {
                ...
                CURRENT_STATE.PC = cpu_cand1;
                // latch_update(&CURRENT_STATE.PC,cpu_cand1, sizeof(uint32_t));
                latch_update(&CURRENT_STATE.PIPE[ID_STAGE], cur_pc, sizeof(uint32_t));
            }

        }
            instruction* inst = get_inst_info(cur_pc);
...
void ID_Stage()
{
    if(CURRENT_STATE.STALL_FOR_DCACHE == TRUE) return;
...
void MEM_Stall()
{
    ...
    {
        bool success;
        if(CURRENT_STATE.STALL_FOR_DCACHE == FALSE)
        {
            CURRENT_STATE.MEM_STALL_ALU_OUT = CURRENT_STATE.EX_MEM_ALU_OUT;
            CURRENT_STATE.MEM_STALL_W_VALUE = CURRENT_STATE.EX_MEM_W_VALUE;
            CURRENT_STATE.MEM_STALL_DEST = CURRENT_STATE.EX_MEM_DEST;
            success = Use_Cache_SW(CURRENT_STATE.EX_MEM_ALU_OUT, CURRENT_STATE.EX_MEM_W_VALUE);
        }
        else
        {
            success = Use_Cache_SW(CURRENT_STATE.MEM_STALL_ALU_OUT, CURRENT_STATE.MEM_STALL_W_VALUE);
        }
        CURRENT_STATE.STALL_FOR_DCACHE = success? FALSE : TRUE;
        if(CURRENT_STATE.STALL_FOR_DCACHE == FALSE)
        {
            CURRENT_STATE.EX_MEM_ALU_OUT = CURRENT_STATE.MEM_STALL_ALU_OUT;
            CURRENT_STATE.EX_MEM_W_VALUE = CURRENT_STATE.MEM_STALL_W_VALUE;
            CURRENT_STATE.EX_MEM_DEST = CURRENT_STATE.MEM_STALL_DEST;
        }

    }
    else if(CURRENT_STATE.EX_MEM_MEM_READ)
    {
        struct cache_data value;
        uint32_t offset;
        if(CURRENT_STATE.STALL_FOR_DCACHE == FALSE)
        {
            CURRENT_STATE.MEM_STALL_ALU_OUT = CURRENT_STATE.EX_MEM_ALU_OUT;
            CURRENT_STATE.MEM_STALL_W_VALUE = CURRENT_STATE.EX_MEM_W_VALUE;
            CURRENT_STATE.MEM_STALL_DEST = CURRENT_STATE.EX_MEM_DEST;
            value =  Use_Cache_LW(CURRENT_STATE.EX_MEM_ALU_OUT);
            offset = CURRENT_STATE.EX_MEM_ALU_OUT & 0x4;
        }
        else
        {
            value = Use_Cache_LW(CURRENT_STATE.MEM_STALL_ALU_OUT);
            offset= CURRENT_STATE.MEM_STALL_ALU_OUT & 0x4;
        }
        CURRENT_STATE.STALL_FOR_DCACHE = value.check? FALSE : TRUE;
        if(CURRENT_STATE.STALL_FOR_DCACHE == FALSE)
        {
            CURRENT_STATE.EX_MEM_ALU_OUT = CURRENT_STATE.MEM_STALL_ALU_OUT;
            CURRENT_STATE.EX_MEM_W_VALUE = CURRENT_STATE.MEM_STALL_W_VALUE;
            CURRENT_STATE.EX_MEM_DEST = CURRENT_STATE.MEM_STALL_DEST;
            uint32_t block = offset ? value.data[1] : value.data[0];
            latch_update(&CURRENT_STATE.MEM_WB_MEM_OUT, block, sizeof(uint32_t));
            latch_update(&CURRENT_STATE.MEM_WB_FORWARD_VALUE, block, sizeof(uint32_t));
        }
